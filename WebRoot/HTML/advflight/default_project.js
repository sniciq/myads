Ext.onReady(function() {
				...
				items: [
					{	
			        	...
			        	items: [
			        	        ...
			        	        { xtype: 'hidden', name: 'isDefault', hidden:true, hiddenLabel:true}
			        	] 
			        }
				]
			},
		...
		buttons: [
			{
				...
				handler: function() {
					...
					if (ditchId == "" && saleId == "") {
						...
						/* 设置默认执行单 */
						edit_form.getForm().findField("isDefault").setValue(1);
							...
							items: [
								new Ext.form.ComboBox({
									...
									store: new Ext.data.SimpleStore({
										fields: ['value', 'text'],
										data: [['', ''],['0', '草稿项目单'],['1','待一审项目单'],['2', '客户待确认项目单'],['3','待二审项目单'],
										['4','退回项目单'],['5','未通过项目单'],['7','执行中项目单']]
									})
			        	       })
							]
					},
	...
	var cm = new Ext.grid.ColumnModel([
	    ...
	    {header:'执行单状态', dataIndex:'bussinessStatus', sortable:true, renderer:rendStauts},
	    ...
	    {header:'操作', dataIndex:'op', renderer: renderOp,width:180, align:'left'}
	]);
	
	function renderOp(value, cellmeta, record, rowIndex, columnIndex, stor) {
		var id = record.data.id;
		var name = record.data.name;
		var bussinessStatus = record.data.bussinessStatus;
		var delStr = '<a href="#" onclick=\"del_breif(\''+id+'\', \'' + name + '\', \'' + bussinessStatus + '\');\">删除</a>';
		var updateStr = '<a href="#" onclick=\"update_breif(\''+id+'\');\">编辑</a>';
		var advflight = '<a href="/myads/HTML/book/book.jsp?priority=0&projectId='+id+'">排期</a>';
		...
		if (bussinessStatus == 1 || bussinessStatus == 3)
			return audit + '&nbsp;&nbsp;&nbsp;&nbsp;' + pointPreview;
		else if (bussinessStatus != 5)
			return advflight + '&nbsp;&nbsp;&nbsp;&nbsp;' + pointPreview + '&nbsp;&nbsp;&nbsp;&nbsp;' + updateStr + del;
		else
			return pointPreview + del;
	}
	...
	window.del_breif = function(id, name, bussinessStatus) {
				   	...
				   	success:function(resp){
				    	var obj=Ext.util.JSON.decode(resp.responseText);
				      	if(obj.result == 'success') {
				      		...
				      		Ext.MessageBox.alert('提示', '执行中的执行单不允许删除,如要删除请先编辑排期信息后在做删除操作!');
				      	} else if (obj.result == 'error') {
				      		Ext.MessageBox.alert('提示', '删除失败!');
				      	} else if (obj.result == 'exist') {
				      		Ext.MessageBox.alert('提示', '删除失败,该执行单下存在有历史排期!');
				      	} else {
				      	}
				      	grid.getStore().reload();
				    },
	...
	var ds = new Ext.data.Store({
		proxy: new Ext.data.HttpProxy({url: '/myads/HTML/advflight/ProjectAction_showAll.action?d=1'}),
		remoteSort: false,
		reader: new Ext.data.JsonReader(
			{
				totalProperty: 'total',
