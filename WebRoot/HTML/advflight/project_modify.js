Ext.onReady(function() {
	...
	var cm = new Ext.grid.ColumnModel([
	    ...
	    {header:'操作', dataIndex:'op', renderer: renderOp,width:230, align:'left'}
	]);
	...
	function renderOp(value, cellmeta, record, rowIndex, columnIndex, stor) {
		...
		var reason = '<a href="#" onclick=\"show_Reason(\''+id+'\');\">未通过原因</a>';
		...
		return advflight + '&nbsp;&nbsp;&nbsp;&nbsp;' + pointPreview + '&nbsp;&nbsp;&nbsp;&nbsp;' + reason + '&nbsp;&nbsp;&nbsp;&nbsp;' + updateStr + del;
	}
	...
	window.show_Reason = function(id) {
		Ext.Ajax.request({
			method: 'post',
			url: '/myads/HTML/advflight/ProjectAction_getProjectVerifyReasonDetail.action',
		   	success:function(resp){
		    	var obj=Ext.util.JSON.decode(resp.responseText);
		    	Ext.getCmp("reasonValue").getEl().dom.innerHTML = obj.data.reason;
		    	reason_win.show();
		    },
		   	params: {projectId: id}
		});
	...
	var ds = new Ext.data.Store({
		proxy: new Ext.data.HttpProxy({url: '/myads/HTML/advflight/ProjectAction_showAll.action?p=71'}),
		remoteSort: false,
