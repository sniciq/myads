Ext.onReady(function() {
	...
	window.verify_approve = function(id, name) {
		Ext.MessageBox.confirm('提示',name + '审核通过？', function(btn) {
			if(btn == 'yes') {
				Ext.Ajax.request({
					method: 'post',
					url: '/myads/HTML/verify/ProjectVerifyAction_verifyCustomer.action',
				   	success:function(resp){
	...
	window.verify_rejected = function(id) {
		Ext.Ajax.request({
			method: 'post',
			url: '/myads/HTML/verify/ProjectVerifyAction_back.action',
		   	success:function(resp){
		   			var obj = Ext.util.JSON.decode(resp.responseText);
		   			if (obj.result == 'success')
		   			{
	...
	var ds = new Ext.data.Store({
		proxy: new Ext.data.HttpProxy({url: '/myads/HTML/advflight/ProjectAction_showAll.action?p=1'}),
		remoteSort: false,
		reader: new Ext.data.JsonReader(
			{
				totalProperty: 'total',
