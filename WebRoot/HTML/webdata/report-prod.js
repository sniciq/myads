Ext.onReady(function() {
	...
	var cm = new Ext.grid.ColumnModel([
   	...
   	var ds = new Ext.data.Store({
   		proxy: new Ext.data.HttpProxy({url: '/myads/HTML/webdata/FlowDataAction_searchProdReport.action'}),
   		remoteSort: true,
			...
			items: [
				'-',
				{
					...
					handler: function() {
						...
						var dayTime = searchForm.getForm().findField("startTime").getRawValue();
						var endTime = searchForm.getForm().findField("endTime").getRawValue();
						window.location.href = '/myads/HTML/webdata/FlowDataAction_searchProdReport.action?exp_name=prod.xls&exp_column_names=' + encodeURI(encodeURI(strColoumnNames)) + '&exp_column_indexs=' + strColoumnIndexs + '&startTime=' + dayTime + "&endTime=" + endTime;
					}
				}
		    ]
	    })
