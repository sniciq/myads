	...
	function openPdWin() {//频道选择对话框
		if(!pdWin) {
			...
			prodNameTree = new Ext.tree.TreePanel({
				...
				border : false,
				autoScroll : true, 
				loader: new Ext.tree.TreeLoader({dataUrl: '/myads/HTML/webdata/ConstantAction_getProdNameList.action'}),
	...
	function openAreaWin() {//地区选择对话框
		if(!areaWin) {
			...
			var allAreaTree = new Ext.tree.TreePanel({
				...
				border : false,
				autoScroll : true, 
				loader: new Ext.tree.TreeLoader({dataUrl: '/myads/HTML/webdata/ConstantAction_getDimAreaList.action'}),
				listeners: {  
					'dblclick':function(node,e){ 
...
//					}
				...
				buttons: [
					{
						...
						handler: function() {
							...
							searchForm.getForm().findField("areaIdList").setValue(idsStr);
							areaWin.hide();
						}
					},
					{
		            ...
		            {	
		            	...
		            	items: [
			            	pdArea = new Ext.form.TextArea({
								height: 50, width:'100%', name:'prodNameList', emptyText: '默认全部频道',disabled:true
							})
		            	]
		            }
				]
			},
			{
				...
				items: [
					{	
		            	...
		            	items: [
		            	        { xtype: 'hidden', id:'prodIdList', name: 'prodIdList', hidden:true, hiddenLabel:true}
		            	] 
			        },
			        {	
						columnWidth: .01, layout: 'form',
		            	items: [
		            	        { xtype: 'hidden', name: 'areaIdList', hidden:true, hiddenLabel:true}
		            	] 
			        }
				]
			},
			...
			},
			{
				...
				items: [
					dqArea = new Ext.form.TextArea({
						height: 50, width:'100%', name:'areaNameList', emptyText: '默认全部地区',disabled:true
					})
				]
			}
		],
	...
	var cm = new Ext.grid.ColumnModel([
	    {header:'时期', dataIndex:'day_id', sortable:true},
	    {header:'频道', dataIndex:'prod_name', sortable:true},
	    {header:'地区', dataIndex:'city', sortable:true},
	    {header:'PV', dataIndex:'pv', sortable:true},
	    {header:'UV', dataIndex:'uv', sortable:true}
	]);
	
	var ds = new Ext.data.Store({
		proxy: new Ext.data.HttpProxy({url: '/myads/HTML/webdata/FlowDataAction_search.action'}),
		remoteSort: true,
			...
			items: [
				'-',
				{
					...
					handler: function() {
						...
						window.location.href = '/myads/HTML/webdata/FlowDataAction_search.action?exp_name=flowdata.xls&exp_column_names=' + encodeURI(encodeURI(strColoumnNames)) 
