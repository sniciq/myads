Ext.onReady(function() {
	...
	var cm = new Ext.grid.ColumnModel([
   	    {header:'星期', dataIndex:'weekDayStr', sortable:false},
   	...
   	var ds = new Ext.data.Store({
   		proxy: new Ext.data.HttpProxy({url: '/myads/HTML/webdata/FlowDataAction_searchReportWeekDay.action'}),
   		remoteSort: true,
			...
			items: [
				'-',
				{
					...
					handler: function() {
						...
						var dayTime = searchForm.getForm().findField("startTime").getRawValue();
						var endTime = searchForm.getForm().findField("endTime").getRawValue();
						window.location.href = '/myads/HTML/webdata/FlowDataAction_searchReportWeekDay.action?exp_name=hour.xls&exp_column_names=' + encodeURI(encodeURI(strColoumnNames)) + '&exp_column_indexs=' + strColoumnIndexs + '&startTime=' + dayTime + "&endTime=" + endTime;
					}
				}
		    ]
	    })
